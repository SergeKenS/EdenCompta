...
@Entity
@Table(name = "other_sessions")
public class OtherSession extends AuditableEntity {
    ...
    @OneToMany(mappedBy = "otherSession", cascade = CascadeType.ALL, orphanRemoval = true)
    private List<SessionMovement> movements = new ArrayList<>();

    public OtherSession() {}

    public OtherSession(Store store, String userId, Money initialAmount) {
        this.store = store;
        this.userId = userId;
