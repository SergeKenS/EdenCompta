...
public class MobileSession extends AuditableEntity {
    ...
    @OneToMany(mappedBy = "mobileSession", cascade = CascadeType.ALL, orphanRemoval = true)
    private List<SessionMovement> movements = new ArrayList<>();

    public MobileSession() {}

    public MobileSession(Store store, String userId, Money initialAmount) {
        this.store = store;
        this.userId = userId;
    ...
    public void addMovement(SessionMovement movement) {
        ...
        movement.setMobileSession(this);
        recalculateTotals();
    }
